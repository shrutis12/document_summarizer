...
public class CosineSimilarity2 {
    ...
    static void calculateSemanticSimilarity() throws Exception {
        TreeMap<Integer, String> sentenceMap1 = new TreeMap<>();  //sentenceMap stores id and corresponding sentence
        Class.forName("com.mysql.jdbc.Driver");
        Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/summarizer", "root", "1234");
        ...
        TreeMap<Integer, String> sentenceMap2 = new TreeMap<>();  //sentenceMap2 stores id and corresponding sentence
        sentenceMap2.putAll(sentenceMap1);

        int i=0,j=0;
        for (Map.Entry<Integer,String> map1:sentenceMap1.entrySet()) {
            j=0;
            double[] vector1=null;
            double[] vector2 =null;
            for (Map.Entry<Integer,String> map2:sentenceMap2.entrySet()) {
                if(j<i+1)
                {
                    j++;
                }
                ...
                {
                    String sentence1=map1.getValue().toLowerCase(); // extract sentence 1 from first map
                    String sentence2=map2.getValue().toLowerCase(); // extract next sentence from second map
                   ...
                   // System.out.println(words3.length+" union is :"+Arrays.toString(words3));
                    ...
                    //threshold=0.5;
                    if(val>=threshold){
                        insertCosinePair(map1.getKey(),map2.getKey(),val);
                    }
    ...
    public static double cosineSimilarity(double[] vector1,double[] vector2)
    {
        ...
        int length= min(vector1.length,vector2.length);
    ...
    public static double compute(String word1, String word2) {
        ...
        double value = new WuPalmer(new CosineSimilarity2().db).calcRelatednessOfWords(word1,word2);
        return value;
    }
